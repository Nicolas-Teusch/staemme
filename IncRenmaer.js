...
win.ScriptAPI.register('240-Inc-Renamer', true, 'Get Drunk', 'support-nur-im-forum@die-staemme.de');
...
function labelAttack() {
  ...
  var oTexts = [
    ...
    /* 7 */
    {
      val: '',
      name: '{sent}',
      info: 'Abschickzeit des Incs(AbhÃƒÂ¤ngig vom gewÃƒÂ¤hlten Truppentyp)',
    },
    /* 8 */
    {
      val: '',
  ...
  $(aTabLZ)
    .find('td:first')
    .append(aTogle);
  /* Links fÃ¼r VP und Inno-Umbenennung ggf. ausblenden */
  ...
  /* Zeilen fÃ¼r die Einheiten erzeugen */
  ...
  function fTabEinstellungen() {
    /* Tabelle fÃ¼r die Einstellungen (Optionen) erstellen*/
    ...
    /* Tabelle fÃ¼r die Einstellungen (Textfelder) erstellen*/
    ...
    inButton.onclick = function () {
      ...
      if ($('#irInUnitNames').val() == 'reset' + world_id) {
        ...
        win.UI.InfoMessage(nameStorage + ' gelÃƒÂ¶scht', 4000, 'success');
        location.reload();
        return;
      }
      /* ÃƒÅ“bergabe der Werte  */
      ...
      if (tre == tre.replace(/[AaGgSsBbLlRrKkPpMmHhCcNnÃƒâ€žÃƒâ€“ÃƒÅ“ÃƒÅ¸ÃƒÂ¤ÃƒÂ¶ÃƒÂ¼+"']/g, '')) {
  ...
  /* Tabelle fÃ¼r die Schnellwahl erstellen */
